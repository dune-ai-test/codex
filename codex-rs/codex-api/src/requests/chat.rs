                    ...
                    for c in content {
                        match c {
                            ...
                            ContentItem::InputAudio { audio_url } => {
                                items.push(
                                    json!({"type":"input_audio","input_audio": {"url": audio_url}}),
                                );
                            }
                        }
                    }
...
let mapped: Vec<Value> = items
                        ...
                        .filter_map(|it| match it {
                            ...
                            FunctionCallOutputContentItem::InputAudio { audio_url } => {
                                Some(json!({"type":"input_audio","input_audio": {"url": audio_url}}))
                            }
                            FunctionCallOutputContentItem::EncryptedContent { .. } => None,
                        })
                        .collect();
                        json!(mapped)
                    } else if let Some(content) = output.text_content() {
                        json!(content)
                    } else {
                        json!("")
                    };
...
mod tests {
    ...
    #[test]
    fn attaches_session_and_subagent_headers() {
