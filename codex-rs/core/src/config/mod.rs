...
///
/// This fork always defaults to the built-in Kilo AI gateway provider so the
/// CLI works out of the box with a Kilo key and no ChatGPT login. The key
/// itself comes from the `KILO_API_KEY` environment variable or the key
/// captured by the CLI's first-run setup; the OpenAI provider remains
/// available via an explicit `model_provider` override.
fn default_model_provider_id(_kilo_api_key_configured: bool) -> &'static str {
    KILO_PROVIDER_ID
}
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let model_provider_id = model_provider
            ...
            .unwrap_or_else(|| default_model_provider_id(/*kilo_api_key_configured*/ false).to_string());
